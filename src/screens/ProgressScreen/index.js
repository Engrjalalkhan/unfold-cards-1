...
const ProgressScreen = ({ onBack }) => {
  ...
  const handleSharePDF = async () => {
    try {
      ...
      // Create PDF using react-native-pdf-lib
      ...
      console.log('📊 Creating 5-page PDF with all charts...');
      
      // ============ PAGE 1: OVERVIEW ============
      const page1 = pdfDoc.addPage([612, 792]);
      let yPosition = 720;
      
      page1.drawText('Complete Streak Progress Report', {
        x: 50,
        y: yPosition,
        size: 20,
      ...
      page1.drawText(`Generated: ${currentDate}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      page1.drawText(`Current Streak: ${streakData.currentStreak} days`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      page1.drawText(`Longest Streak: ${streakData.longestStreak} days`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      page1.drawText(`Total Submissions: ${streakData.totalDays}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      // ============ PAGE 2: DAILY LINE CHART ============
      const page2 = pdfDoc.addPage([612, 792]);
      yPosition = 720;
      
      page2.drawText('DAILY LINE CHART', {
        x: 50,
        y: yPosition,
        size: 20,
        font: helveticaBoldFont,
      ...
      page2.drawText(`Generated: ${currentDate}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      // Daily Line Chart
      const dailyMaxValue = Math.max(...streakData.dailyData.map(d => d.total), 1);
      const dailyChartHeight = 120;
      const dailyChartWidth = 450;
      const dailyChartX = 50;
      const dailyChartY = yPosition - dailyChartHeight;
      ...
      // Draw bars for daily data
      const dailyBarWidth = dailyChartWidth / streakData.dailyData.length;
      streakData.dailyData.forEach((day, index) => {
        const barHeight = (day.total / dailyMaxValue) * dailyChartHeight;
        const barX = dailyChartX + (index * dailyBarWidth) + 5;
        const barY = dailyChartY;
        
        // Draw stacked bars
        let currentY = barY;
        const dailyHeight = (day.daily / dailyMaxValue) * dailyChartHeight;
        const moodHeight = (day.mood / dailyMaxValue) * dailyChartHeight;
        const discoverHeight = (day.discover / dailyMaxValue) * dailyChartHeight;
        
        // Daily bar (bottom)
        page2.drawRectangle({
          x: barX,
          y: currentY,
          width: dailyBarWidth - 10,
          height: dailyHeight,
          color: rgb(0.545, 0.412, 0.965), // Purple
        });
        currentY += dailyHeight;
        ...
        page2.drawRectangle({
          ...
          width: dailyBarWidth - 10,
          height: moodHeight,
          color: rgb(0.231, 0.51, 0.965), // Blue
        });
        currentY += moodHeight;
        ...
        page2.drawRectangle({
          ...
          color: rgb(0.063, 0.596, 0.506), // Green
        });
        
        // Day label
        page2.drawText(day.day.substring(0, 3), {
          x: barX,
          y: dailyChartY - 15,
          size: 8,
          font: helveticaFont,
      ...
      yPosition = dailyChartY - 40;
      
      // Daily data breakdown
      page2.drawText('Daily Data Breakdown:', {
        x: 50,
        y: yPosition,
        size: 14,
        font: helveticaBoldFont,
      ...
      streakData.dailyData.forEach(day => {
        if (yPosition < 50) {
          const newPage = pdfDoc.addPage([612, 792]);
          yPosition = 720;
        }
        
        page2.drawText(`${day.day}: Daily=${day.daily}, Mood=${day.mood}, Discover=${day.discover}, Total=${day.total}`, {
          x: 50,
          y: yPosition,
          size: 10,
          font: helveticaFont,
      ...
      // ============ PAGE 3: WEEKLY CIRCLE CHART ============
      const page3 = pdfDoc.addPage([612, 792]);
      yPosition = 720;
      
      page3.drawText('WEEKLY CIRCLE CHART', {
        x: 50,
        y: yPosition,
        size: 20,
        font: helveticaBoldFont,
      ...
      page3.drawText(`Generated: ${currentDate}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      // Weekly totals
      const weeklyTotals = {
        Daily: streakData.weeklyData.reduce((sum, w) => sum + w.daily, 0),
        Mood: streakData.weeklyData.reduce((sum, w) => sum + w.mood, 0),
        Discover: streakData.weeklyData.reduce((sum, w) => sum + w.discover, 0)
      };
      
      const weeklyTotal = Object.values(weeklyTotals).reduce((sum, val) => sum + val, 0);
      
      // Draw circle chart representation
      const circleRadius = 80;
      const circleCenterX = 200;
      const circleCenterY = yPosition - 100;
      ...
      // Draw colored segments (simplified as concentric circles)
      let currentRadius = circleRadius;
      const colors = [
        { key: 'Daily', value: weeklyTotals.Daily, color: rgb(0.545, 0.412, 0.965) },
        { key: 'Mood', value: weeklyTotals.Mood, color: rgb(0.231, 0.51, 0.965) },
        { key: 'Discover', value: weeklyTotals.Discover, color: rgb(0.063, 0.596, 0.506) }
      ];
      
      colors.forEach((segment, index) => {
        const segmentRadius = (segment.value / weeklyTotal) * circleRadius;
        page3.drawCircle({
          x: circleCenterX,
          y: circleCenterY,
          size: segmentRadius,
      ...
      // Legend
      yPosition = circleCenterY - 120;
      colors.forEach((segment) => {
        const percentage = weeklyTotal > 0 ? (segment.value / weeklyTotal) * 100 : 0;
        ...
        page3.drawText(`${segment.key}: ${segment.value} (${percentage.toFixed(1)}%)`, {
          x: 375,
          y: yPosition,
          size: 12,
          font: helveticaFont,
      ...
      page3.drawText('Weekly Data Breakdown:', {
        x: 50,
        y: yPosition,
        size: 14,
        font: helveticaBoldFont,
      ...
      streakData.weeklyData.forEach(week => {
        if (yPosition < 50) {
          const newPage = pdfDoc.addPage([612, 792]);
          yPosition = 720;
        }
        
        page3.drawText(`${week.week}: Daily=${week.daily}, Mood=${week.mood}, Discover=${week.discover}, Total=${week.total}`, {
          x: 50,
          y: yPosition,
          size: 10,
          font: helveticaFont,
      ...
      // ============ PAGE 4: MONTHLY PIE CHART ============
      const page4 = pdfDoc.addPage([612, 792]);
      yPosition = 720;
      
      page4.drawText('MONTHLY PIE CHART', {
        x: 50,
        y: yPosition,
        size: 20,
        font: helveticaBoldFont,
      ...
      page4.drawText(`Generated: ${currentDate}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      // Monthly totals
      const monthlyTotals = {
        Daily: streakData.monthlyData.reduce((sum, m) => sum + m.daily, 0),
        Mood: streakData.monthlyData.reduce((sum, m) => sum + m.mood, 0),
        Discover: streakData.monthlyData.reduce((sum, m) => sum + m.discover, 0)
      };
      
      const monthlyTotal = Object.values(monthlyTotals).reduce((sum, val) => sum + val, 0);
      
      // Draw pie chart
      const pieRadius = 80;
      const pieCenterX = 200;
      const pieCenterY = yPosition - 100;
      
      // Draw pie segments as rectangles (simplified)
      let pieY = pieCenterY - pieRadius;
      const monthlyColors = [
        { key: 'Daily', value: monthlyTotals.Daily, color: rgb(0.545, 0.412, 0.965) },
        { key: 'Mood', value: monthlyTotals.Mood, color: rgb(0.231, 0.51, 0.965) },
        { key: 'Discover', value: monthlyTotals.Discover, color: rgb(0.063, 0.596, 0.506) }
      ];
      
      monthlyColors.forEach((segment) => {
        const segmentHeight = (segment.value / monthlyTotal) * (pieRadius * 2);
        page4.drawRectangle({
          x: pieCenterX - pieRadius,
          y: pieY,
          width: pieRadius * 2,
      ...
      // Legend
      yPosition = pieCenterY - 120;
      monthlyColors.forEach((segment) => {
        const percentage = monthlyTotal > 0 ? (segment.value / monthlyTotal) * 100 : 0;
        ...
        page4.drawText(`${segment.key}: ${segment.value} (${percentage.toFixed(1)}%)`, {
          x: 375,
          y: yPosition,
          size: 12,
          font: helveticaFont,
      ...
      page4.drawText('Monthly Data Breakdown:', {
        x: 50,
        y: yPosition,
        size: 14,
        font: helveticaBoldFont,
      ...
      streakData.monthlyData.forEach(month => {
        if (yPosition < 50) {
          const newPage = pdfDoc.addPage([612, 792]);
          yPosition = 720;
        }
        
        page4.drawText(`${month.month}: Daily=${month.daily}, Mood=${month.mood}, Discover=${month.discover}, Total=${month.total}`, {
          x: 50,
          y: yPosition,
          size: 10,
          font: helveticaFont,
      ...
      // ============ PAGE 5: YEARLY DONUT CHART ============
      const page5 = pdfDoc.addPage([612, 792]);
      yPosition = 720;
      
      page5.drawText('YEARLY DONUT CHART', {
        x: 50,
        y: yPosition,
        size: 20,
        font: helveticaBoldFont,
      ...
      page5.drawText(`Generated: ${currentDate}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      // Yearly totals
      const yearlyTotals = {
        Daily: streakData.yearlyData.reduce((sum, y) => sum + y.daily, 0),
        Mood: streakData.yearlyData.reduce((sum, y) => sum + y.mood, 0),
        Discover: streakData.yearlyData.reduce((sum, y) => sum + y.discover, 0)
      };
      
      const yearlyTotal = Object.values(yearlyTotals).reduce((sum, val) => sum + val, 0);
      
      // Draw donut chart
      const donutOuterRadius = 80;
      const donutInnerRadius = 40;
      const donutCenterX = 200;
      const donutCenterY = yPosition - 100;
      
      // Draw outer circle
      page5.drawCircle({
        x: donutCenterX,
        y: donutCenterY,
        size: donutOuterRadius,
        color: rgb(0.95, 0.95, 0.95),
        borderColor: rgb(0.8, 0.8, 0.8),
        borderWidth: 1,
      });
      
      // Draw colored segments as arcs (simplified as rectangles)
      const yearlyColors = [
        { key: 'Daily', value: yearlyTotals.Daily, color: rgb(0.545, 0.412, 0.965) },
        { key: 'Mood', value: yearlyTotals.Mood, color: rgb(0.231, 0.51, 0.965) },
        { key: 'Discover', value: yearlyTotals.Discover, color: rgb(0.063, 0.596, 0.506) }
      ];
      
      let donutY = donutCenterY - donutOuterRadius;
      yearlyColors.forEach((segment) => {
        const segmentHeight = (segment.value / yearlyTotal) * (donutOuterRadius * 2);
        page5.drawRectangle({
          x: donutCenterX - donutOuterRadius,
          y: donutY,
          width: donutOuterRadius * 2,
          height: segmentHeight,
          color: segment.color,
        });
        donutY += segmentHeight;
      });
      ...
      // Legend
      yPosition = donutCenterY - 120;
      yearlyColors.forEach((segment) => {
        const percentage = yearlyTotal > 0 ? (segment.value / yearlyTotal) * 100 : 0;
        ...
        page5.drawText(`${segment.key}: ${segment.value} (${percentage.toFixed(1)}%)`, {
          x: 375,
          y: yPosition,
          size: 12,
          font: helveticaFont,
      ...
      page5.drawText('Yearly Data Breakdown:', {
        x: 50,
        y: yPosition,
        size: 14,
        font: helveticaBoldFont,
      ...
      streakData.yearlyData.forEach(year => {
        if (yPosition < 50) {
          const newPage = pdfDoc.addPage([612, 792]);
          yPosition = 720;
        }
        
        page5.drawText(`${year.year}: Daily=${year.daily}, Mood=${year.mood}, Discover=${year.discover}, Total=${year.total}`, {
          x: 50,
          y: yPosition,
          size: 10,
          font: helveticaFont,
      ...
      if (streakData.longestStreak > streakData.currentStreak) {
        page5.drawText(`🏆 Your longest streak was ${streakData.longestStreak} days. You can beat it!`, {
          x: 50,
          y: yPosition,
          size: 12,
          font: helveticaFont,
      ...
      page5.drawText(`📊 You've completed ${streakData.totalDays} total submissions across all question types.`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      page5.drawText(`Selected Period: ${selectedPeriod}`, {
        x: 50,
        y: yPosition,
        size: 12,
        font: helveticaFont,
      ...
      page5.drawText(
        `Chart Type: ${selectedPeriod === 'daily' ? 'Line Chart' : 
                      selectedPeriod === 'weekly' ? 'Circle Chart' : 
                      selectedPeriod === 'monthly' ? 'Pie Chart' : 'Donut Chart'}`,
        {
          x: 50,
          y: yPosition,
          size: 12,
      ...
      page5.drawText('Generated using Unfold Cards App', {
        x: 50,
        y: yPosition,
        size: 10,
        font: helveticaFont,
      ...
      await Share.share({
        url: pdfUri,
        title: `Complete Charts Progress Report - ${currentDate}`,
        subject: 'Unfold Cards - Complete Charts Progress Report',
        message: `Here's my complete progress report with all charts from Unfold Cards app!`
      });
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sharePdfContainer}>
          ...
          <Text style={styles.pdfInfoText}>
            Generate complete progress report with all charts (Daily, Weekly, Monthly, Yearly)
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
